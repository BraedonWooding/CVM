...
impl<'a> TypeInfer<'a> {
    ...
    fn type_infer_expr(&mut self, expr: &mut Expr) {
        let ty = match &mut expr.kind.inner {
            ...
            ExprKind::Var(id) => match self.stack.cur().borrow_mut().lookup_var_cond(&id) {
            ...
            ExprKind::Member(ref mut expr, id) => {
                ...
                // SO!!!  Maybe we don't actually want to do this
                // and just always type infer later........
                None
            }
            ...
            ExprKind::FuncCall(ref mut func, ref mut args) => {
                ...
                match &func.type_annot {
                    ...
                    ParsedType::Fresh { id } => {
                        ...
                        //       we could also just let the type checker handle
                        //       it (lol)
                        if let ParsedType::Func { ref ret, .. } = *ty {
                            Some((**ret).clone())
                        } else {
                            // type checker should fix this!
                            // for us without any issues
                            None
                        }
                    }
                    _ => {
                        ...
                        warn!(
                            "Can't perform a call on a pointer, array, vec on object {:?}",
                            func
                        );
                        None
                    }
    ...
    fn type_infer_statement(&mut self, statement: &mut Statement) {
        match statement {
            ...
            Statement::For(ref mut start, ref mut cond, ref mut end, ref mut block) => {
                ...
                if let Some(ref mut expr) = cond {
                    self.type_infer_expr(expr);
                }
                if let Some(ref mut expr) = end {
                    self.type_infer_expr(expr);
                }
                self.type_infer_block(block);
                self.stack.pop();
            }
            Statement::Expr(ref mut inner) | Statement::Return(ref mut inner) => {
                self.type_infer_expr(inner)
            }
            Statement::Defer => {}
        }
